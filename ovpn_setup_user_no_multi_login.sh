...
function installQuestions() {
	...
	if [[ $CUSTOMIZE_ENC == "n" ]]; then
		...
		until [[ $CIPHER_CHOICE =~ ^[1-6]$ ]]; do
			read -rp "Cipher [1-6]: " -e -i 4 CIPHER_CHOICE
		done
...
function newClient() {
	...
	cp /etc/openvpn/client-template.txt "/var/www/html/$CLIENT.ovpn"
	{
		echo "<ca>"
	...
	echo ""
	echo "The configuration file has been written to /var/www/html/$CLIENT.ovpn."
